...
export default function ProjectsPage() {
  const [projects, setProjects] = useState<ProjectWithTours[]>([]);
                  ...
                  <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">
                    {project.tours.map((tour) => (
                      <Link
                        key={tour.id}
                        href={`/tours/${tour.id}`}
                        className="flex items-center gap-2.5 p-3 rounded-lg border hover:bg-muted/50 hover:border-primary/30 transition-all"
                      >
